...
termids <- tribble(
  ...
  paste0(
    ...
    "-",
    current_last_year
  ), calc_ps_termid(current_first_year), 8,
  paste0(
    current_first_year - 1,
    "-",
    current_last_year - 1
  ), calc_ps_termid(current_first_year - 1), 7,
  paste0(
    current_first_year - 2,
    "-",
    current_last_year - 2
  ), calc_ps_termid(current_first_year - 2), 6,
  paste0(
    current_first_year - 3,
    "-",
    current_last_year - 3
  ), calc_ps_termid(current_first_year - 3), 5
)
...
storedgrades_8th <- storedgrades %>%
  filter(
    studentid %in% eighth_students$studentid,
...
non_storedgpas_8th <- non_storedgrades_8th %>%
  ...
  group_by(
    studentid, termid, # schoolid   # pgfinalgrades table doesn't include school identifiers
  ) %>%
  summarize(year_gpa = mean(fixscale_gpa_points, na.rm = TRUE)) %>%
  mutate(grade_level = case_when(
    termid == calc_ps_termid(current_first_year - 1) ~ 7,
...
all_gpas %>%
  count(student_number, first_name, last_name) %>%
  filter(n > 1)
