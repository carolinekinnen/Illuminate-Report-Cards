...
ps_upload_letter_grade <- final_grades_final %>% #final_grades %>%
...
ps_upload_final <- ps_upload_percentage %>% 
  ...
  mutate(schoolname = case_when(
    schoolname == "KIPP Ascend Middle" ~ "KIPP Ascend Middle School",   # need to change so heading will match what is tied to site_id in Powerschool
    TRUE ~ schoolname
  ))
